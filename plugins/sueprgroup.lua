local function check_member_super(cb_extra, success, result) 
  ...
  if success == 0 then 
    send_large_msg(receiver, "فقط الادمن يحق له ذالك🌝") 
  end 
  for k,v in pairs(result) do 
    ...
    if member_id ~= our_id then 
      ...
      save_data(_config.moderation.data, data) 
      local text = 'تْـ⚠️ـم تْفَـ؏ـيَلُ الُـ♻️ـبّوَتْ دِاٌخـ🔰ـلُ الُمٌجْـ✔️ـمٌوَْ؏ـه🌝' 
...
local function check_member_superrem(cb_extra, success, result) 
  ...
  for k,v in pairs(result) do 
    ...
    if member_id ~= our_id then 
      ...
      save_data(_config.moderation.data, data) 
      local text = '❎تــم تہعطیےہل الہمہجمہوعہھ ❎'
      return reply_msg(msg.id, text, ok_cb, false) 
    end 
  end 
end 
...
local function lock_group_links(msg, data, target) 
  ...
  if group_link_lock == 'yes' then 
    return 'الروابط مقفوله😡🔐 لا تصير مطي😂😘❤️🔒\n🔺Order By : @'..msg.from.username..'\n🔻Order By : '.. msg.from.id..'\n' 
  else 
    data[tostring(target)]['settings']['lock_link'] = 'yes' 
    save_data(_config.moderation.data, data) 
    return 'تم قفل الروابط😠🔒بعد اليدز رابط اطلعه من الكروب ﭼـلاليق😂❤️☑️\n🔺Order By : @'..msg.from.username..'\n🔻Order By : '.. msg.from.id..'\n' 
  end 
end 
...
local function unlock_group_links(msg, data, target) 
  ...
  if group_link_lock == 'no' then 
    ...
    save_data(_config.moderation.data, data) 
    return 'تم فتح الروابط😒🙄🔓دز رابط بسرعه علمود يقفلونهن🔒😘❤️😽\n🔺Order By : @'..msg.from.username..'\n🔻Order By : '.. msg.from.id..'\n' 
  end 
end 
...
local function promote2(receiver, member_username, user_id) 
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username 
  save_data(_config.moderation.data, data) 
  send_large_msg(receiver, member_username..'تم رفعك من مطي الى قرد 👍😘 شد حيلك يرفعونك انسان😂❤️') 
end 
...
function get_message_callback(extra, success, result) 
    ...
    elseif get_cmd == "رفع اداري" then 
        ...
        if result.from.username then 
            text = "@"..result.from.username.."تم رفعك من قرد الى انسان😍❤️" 
        else 
            text = "[ "..user_id.." ]   تم رفعك من قرد الى انسان😍❤️ " 
        end 
    ...
    elseif get_cmd == "تنزيل اداري" then 
        ...
        if result.from.username then 
            text = "@"..result.from.username.."تم تنزيلك مطي لأنك مطي وراح تبقه مطي وما يصير براسك خير😒❤️" 
        else 
            text = "[ "..user_id.." ] تم تنزيلك مطي لأنك مطي وراح تبقه مطي وما يصير براسك خير😒❤️ " 
        end 
...
send_large_msg(receiver, "["..user_id.."] يله احجي 😂🖐 راح الكتم منك😘❤️ ") 
        elseif is_admin1(msg) then 
            mute_user(chat_id, user_id) 
            send_large_msg(receiver, " ["..user_id.."] اشكد تحجي ولك هسه سوولك كتم ارتاحيت🤒☝️") 
        end 
    end 
end 
